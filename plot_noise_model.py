...
if args.data:
    ...
    ax1.set_ylabel(r'$V/\sqrt{Hz}$')
    ax1.set_xlabel("Frequency (Hz)")
    if logy:
        ax1.set_yscale(u'log') 
    if logx:
        ax1.set_xscale(u'log')
        ax1.text(0.1, 0.9, "Output noise", transform = ax1.transAxes)
    fig.legend()
    fig.savefig("noise_output_data_vs_model",bbox_inches='tight')
    ...
    ax21.text(0.1, 0.9, "Difference b/w data and model", transform=ax21.transAxes)
    ax21.set_ylabel(r'data-model [$V/\sqrt{Hz}$]')
    ...
    ax22.text(0.1, 0.9, "Relative difference", transform=ax22.transAxes)
    ax22.set_ylabel("(data-model)/model [arb. units]")
    ax22.set_xlabel("Frequency (Hz)")
    #ax22.set_yscale(u'log') 
    ax22.set_xscale(u'log') 
    
    fig2.savefig("noise_output_data_vs_model_diff",bbox_inches='tight')
